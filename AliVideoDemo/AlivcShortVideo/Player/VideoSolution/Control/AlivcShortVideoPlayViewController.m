...
@interface AlivcShortVideoPlayViewController ()<AlivcShortVideoElasticViewDelegate,AlivcShortVideoMenuViewDelegate,AMDDelegate,AlivcShortVideoProgressDelegate,AlivcShortVideoPublishManagerDelegate,UICollectionViewDelegate>
...
- (void)viewDidLoad {
    ...
    [self loadData];
    //设置log等级
    Class sdkInfo = NSClassFromString(@"AliyunVideoSDKInfo");
    if (sdkInfo) {
        [sdkInfo performSelector:NSSelectorFromString(@"setLogLevel:") withObject:@(2)];
    }
}
...
- (void)showPhotoLibUnAuthAlert{
    ...
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"" message:@"请在iPhone的“设置-隐私-照片”选项中，允许访问你的手机相册。" preferredStyle:UIAlertControllerStyleAlert];
...
- (BOOL)judgeIsPublishing{
    if ([AlivcShortVideoPublishManager shared].currentStatus == AlivcPublishStatusPublishing) {
        [MBProgressHUD showMessage:@"视频还在处理中" inView:self.view];
        return YES;
    }else{
        return NO;
    }
...
-(void)alivcShortVideoMenuViewDownloadAction {
    ...
    if(netWorkStatus == AliyunPVNetworkStatusNotReachable) {
        [MBProgressHUD showMessage:@"当前无网络,请连网后重试!" inView:self.view];
        return;
    }
    //判断权限
    __weak typeof(self) weakSelf = self;
...
-(void)alivcShortVideoMenuViewDeleteAction {
    AlivcAlertView *alertView = [[AlivcAlertView alloc]initWithAlivcTitle:nil message:@"确认删除吗?" delegate:self cancelButtonTitle:@"取消" confirmButtonTitle:@"确认"];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        switch (newStatus) {
            ...
            case AlivcPublishStatusFailure:{
                [self.uploadProgress setShowText:@"发布失败"];
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    [self.uploadProgress removeFromSuperview];
                });
                [self showErrorAlertViewWithString:@"网络中断，发布失败"];
            ...
            case AlivcPublishStatusPublishing:{
                [self.uploadProgress setShowText:@"后台发布中"];
            }
...
- (void)publishToAppServerWithDic:(NSDictionary *)paramDic{
    [AlivcQuVideoServerManager quServerVideoPublishWithDic:paramDic success:^{
        ...
        [self.uploadProgress setShowText:@"发布成功"];
        [MBProgressHUD showMessage:@"发布成功，已进入审核通道" inView:self.view];
        ...
        [self showErrorAlertViewWithString:@"网络错误，发布失败"];
        [self.uploadProgress setShowText:@"发布失败"];
...
- (void)showErrorAlertViewWithString:(NSString *)errorString{
    AlivcAlertView *alertView = [[AlivcAlertView alloc]initWithAlivcTitle:nil message:errorString delegate:self cancelButtonTitle:@"关闭" confirmButtonTitle:@"重试"];
    [alertView show];
    [alertView setStyle:AlivcAlertViewStyleWhite];
    alertView.tag = 102;
...
- (void)showInfoAlertViewWithString:(NSString *)errorString{
    AlivcAlertView *alertView = [[AlivcAlertView alloc]initWithAlivcTitle:nil message:errorString delegate:self cancelButtonTitle:nil confirmButtonTitle:@"确定"];
    [alertView show];
    [alertView setStyle:AlivcAlertViewStyleWhite];
}
...
- (void)retryPublish{
    if ([AlivcShortVideoPublishManager shared].currentStatus == AlivcPublishStatusSuccess && _publishParamDic) {
        ...
        [self.uploadProgress setShowText:@"正在重试"];
