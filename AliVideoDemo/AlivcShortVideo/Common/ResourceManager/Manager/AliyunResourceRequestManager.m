...
+ (void)fetchPasterCategoryWithType:(kPasterCategory)pasterCategory
    ...
    NSString *url = [NSString stringWithFormat:@"%@/resource/getPasterInfo",kQPResourceHostUrl];
...
+ (void)fetchPasterListWithType:(kPasterCategory)pasterCategory
    ...
    NSString *url = [NSString stringWithFormat:@"%@/resource/getPasterList",kQPResourceHostUrl];
    [AlivcAppServer getWithUrlString:url parameters:param completionHandler:^(NSString * _Nullable errString, NSDictionary * _Nullable resultDic) {
        if (errString) {
            failure(errString);
        }else{
...
+ (void)fetchCaptionListWithSuccess:(void(^)(NSArray *resourceListArray))success
                            failure:(void(^)(NSString *errorStr))failure {
    NSDictionary *param = @{};
    NSString *url = [NSString stringWithFormat:@"%@/resource/getTextPaster",kQPResourceHostUrl];
    [AlivcAppServer getWithUrlString:url parameters:param completionHandler:^(NSString * _Nullable errString, NSDictionary * _Nullable resultDic) {
        if (errString) {
            failure(errString);
        }else{
...
+ (void)fetchTextPasterListWithTextPasterCategoryId:(NSInteger)textPasterCategoryId
    ...
    NSString *url = [NSString stringWithFormat:@"%@/resource/getTextPasterList",kQPResourceHostUrl];
    [AlivcAppServer getWithUrlString:url parameters:param completionHandler:^(NSString * _Nullable errString, NSDictionary * _Nullable resultDic) {
        if (errString) {
            failure(errString);
        }else{
...
+ (void)fetchMVListSuccess:(void(^)(NSArray *resourceListArray))success
                   failure:(void(^)(NSString *errorStr))failure {
    NSDictionary *param = @{}; 
     NSString *url = [NSString stringWithFormat:@"%@/resource/getMv",kQPResourceHostUrl];
    [AlivcAppServer getWithUrlString:url parameters:param completionHandler:^(NSString * _Nullable errString, NSDictionary * _Nullable resultDic) {
        if (errString) {
            failure(errString);
        }else{
